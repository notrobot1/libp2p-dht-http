...
func handleConnection(net network.Network, conn network.Conn) {
	...
	//We don't want to get public keys from all network members, so we list here the identifiers of the nodes we are interested in
	if conn.RemotePeer().String() == "QmQ2DFAX7qAiak2o4nAT9JEEBUGZPanPhM76GCnzkWLpBH" || conn.RemotePeer().String() == "QmSuYqBsX2qE7KQi8C2fJxiLeTMSEcKMDZjEpeHikRZxK6" {
		cache.Set(conn.RemotePeer().String(), conn.RemotePublicKey())
	}
...
func main() {
	...
	for _, provider := range providers {
		...
		if provider.ID.String() != h.ID().String() {
			...
			sendRequest(provider.ID.String(), h, "POST", "Hello")
